...
<div class="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 py-8">
    <div class="container mx-auto px-4">
        ...
        <div class="text-center mb-8">
            ...
            <h1 class="text-4xl font-serif font-bold text-oxford-navy mb-2">Edit Agenda</h1>
            <p class="text-oxford-gold font-medium">Update event details and information</p>
        </div>
                ...
                <div class="bg-gradient-to-r from-oxford-navy via-oxford-navy-light to-oxford-navy px-8 py-6">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-oxford-gold text-sm">Update the event details below</p>
                        </div>
                    </div>
                </div>
                
                <form action="{{ route('admin.agendas.update', $agenda) }}" method="POST" class="p-8 space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="bg-gray-50 rounded-xl p-6 border border-gray-200">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
                            <div class="space-y-2">
                                ...
                                <input type="time" name="waktu" id="waktu" value="{{ old('waktu', $agenda->waktu) }}"
                                       class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-oxford-gold focus:ring-4 focus:ring-oxford-gold/20 transition-all duration-300 font-medium" 
                                       required>
                                @error('waktu')
                                    <p class="mt-2 text-sm text-red-600 flex items-center">
                    ...
                    <div class="bg-gray-50 rounded-xl p-6 border border-gray-200">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="status" id="status" 
                                    ...
                                    required>
                                <option value="upcoming" class="text-gray-900" {{ old('status', $agenda->status) == 'upcoming' ? 'selected' : '' }}>📅 Upcoming - Event is scheduled</option>
                                <option value="ongoing" class="text-gray-900" {{ old('status', $agenda->status) == 'ongoing' ? 'selected' : '' }}>🔄 Ongoing - Event is happening now</option>
                                <option value="completed" class="text-gray-900" {{ old('status', $agenda->status) == 'completed' ? 'selected' : '' }}>✅ Completed - Event has finished</option>
                            </select>
                            @error('status')
                                <p class="mt-2 text-sm text-red-600 flex items-center">
                                    <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
